...
const handler = async (event) => {
  if (event.Records && event.Records.length > 0) {
    for (const record of event.Records) {
      ...
      const { data, templateName, bucketName, subject } = JSON.parse(body);
      const info = data;
      const htmlContent = await readFile(bucketName, templateName, info);
      await processSentEmail(htmlContent, info.email, subject);
    }
  }
};
